...
export const getNoteById = ({ id }: IdParams): ResultAsync<Note, ServerError> =>
	tryDbTask(() => db.note.findFirst({ where: { id } })).andThen((note) =>
...
export const getNoteById = ({ id }: IdParams): Effect.Effect<Note, ServerError> =>
	Effect.tryPromise({
		...
		Effect.flatMap((note) =>
			note
				? Effect.succeed(note)
				: Effect.fail(createError('RecordNotFound', `Note with id ${id} not found`))
		)
	);
```
...
```ts
export type Result<T, E = ServerError> = { ok: true; value: T } | { ok: false; error: E };
